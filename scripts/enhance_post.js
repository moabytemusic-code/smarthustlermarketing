...
async function enhancePost() {
    ...
    if (isSkeleton) {
        console.log(`📝 Detected SKELETON/EMPTY post: "${path.basename(filePath)}"`);
        console.log(`🤖 Starting AI Author Mode (Cost: ~$0.05)...`);
        ...
        try {
            ...
            console.log(`✅ Success! Wrote full article for: ${path.basename(filePath)}`);
            return;
    ...
    if (originalContent.includes('## Sources') || originalContent.includes('## References')) {
        console.log('⚠️  This post already has content and sources. Skipping.');
        process.exit(0);
    }

    console.log(`🔍 Content exists. switching to "Citation Mode" for: ${path.basename(filePath)}...`);
    ...
    * [Source Title](URL)
    ...
    try {
        ...
        console.log(`✅ Added citations to: ${path.basename(filePath)}`);
